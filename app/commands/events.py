...
class events(commands.Cog):
    ...
    async def on_guild_join(self, guild: discord.Guild):
        ...
        try:
            embed = discord.Embed(title=f"Hello, {guild.name} I am Status Checker",
        ...
        embed = discord.Embed(
            title="I joined a guild", description=f"Guild Name: {guild.name} \n Guild ID: {guild.id} \n Total Guilds: {len(self.bot.guilds)}", color=discord.Color.green())
        if guild.icon != None:
    ...
    async def on_guild_remove(self, guild: discord.Guild):
        ...
        try:
            ...
            embed = discord.Embed(title="Hey there, I am sorry to see you go",
                                  description="All references of your guild have been removed from our database. If you would feel up to, please can you fill in the feedback form below.", color=discord.Color.red())
        ...
        log_channel = self.bot.get_channel(1050359176859222017)
        embed = discord.Embed(
            title="I Left a guild", description=f"Guild Name: {guild.name} \n Guild ID: {guild.id} \n Total Guilds: {len(self.bot.guilds)}", color=discord.Color.red())
        if guild.icon != None:
            embed.set_thumbnail(url=guild.icon)
        embed.set_footer(
            text=f"Guild Owner: {guild.owner.name}#{guild.owner.discriminator}")
